...
local function show_run_menu()
    ...
    local options = {"Nova Shell","programs...","run...","shutdown","restart"}
    local process = processes[process_current]
    ...
    if ret[1] then
        ...
        if ret[2] == 1 then
            ...
            process_run(env, "nova/.sys/shell.lua", {}, "Nova Shell", process.x, process.y)
        elseif ret[2] == 2 then
            
            local pkgItems = {}
            if fs.exists("nova/packages") and fs.isDir("nova/packages") then
                local list = fs.list("nova/packages")
            ...
            if ret2[1] and ret2[2] > 0 then
                ...
                if type(selItem) == "table" and selItem.file then
                    local path = fs.combine("nova/packages", selItem.file)
...
local env = {shell=shell, multishell=multishell_ext}
process_run(env, "nova/.sys/shell.lua", {}, "Nova Shell")
process_set_focus(1)
wm_mainloop()
